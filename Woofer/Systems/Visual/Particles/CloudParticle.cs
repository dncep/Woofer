...
namespace WooferGame.Systems.Visual.Particles
{
    class CloudParticle : Entity
    {
        ...
        public CloudParticle(Vector2D pos, int delay)
        {
            ...
            Components.Add(new Renderable(new Sprite("particles", new Rectangle(-4.5f, -2, 9, 4), new Rectangle(0, 0, 9, 4))));
            ...
            Components.Add(new Physical() { GravityMultiplier = 0.1f });
        }

        public override void Initialize()
        {
            Components.Get<Physical>().Velocity = new Vector2D((float)Owner.Random.NextDouble() * 16, 0).Rotate((float)(Owner.Random.NextDouble() * 2 * Math.PI));
        }
    }
}
