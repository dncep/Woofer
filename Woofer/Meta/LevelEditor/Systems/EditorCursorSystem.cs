...
namespace WooferGame.Meta.LevelEditor.Systems
{
    ...
    class EditorCursorSystem : ComponentSystem
    {
        ...
        public override void Tick()
        {
            ...
            {
                if (CursorPos.X < viewRect.Left) Owner.CurrentViewport.X = CursorPos.X + viewRect.Width / 2;
                if (CursorPos.X > viewRect.Right) Owner.CurrentViewport.X = CursorPos.X - viewRect.Width / 2;
                if (CursorPos.Y < viewRect.Bottom) Owner.CurrentViewport.Y = CursorPos.Y + viewRect.Height / 2;
                if (CursorPos.Y > viewRect.Top) Owner.CurrentViewport.Y = CursorPos.Y - viewRect.Height / 2;
            }
        }
